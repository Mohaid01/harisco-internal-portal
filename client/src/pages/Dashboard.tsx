...
import { Users, Smartphone, Wrench, AlertCircle, Loader2, Activity, Filter, Search, X, Calendar } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ userRole }) => {
    ...
    try {
      ...
      if (userRole === 'STAFF') {
        ...
        setStats({
          employees: 0,
          devices: 0,
          repairs: rep.filter((r: any) => r.status !== 'APPROVED' && r.status !== 'REJECTED').length, // Filter by requester later
        ...
        setStats({
          ...
          repairs: rep.filter((r: any) => r.status !== 'APPROVED' && r.status !== 'REJECTED').length,
  ...
  const statCards = [
    { label: 'Total Employees', value: stats.employees, icon: <Users className="text-blue-600" />, trend: 'Active users', roles: ['IT', 'DIRECTOR', 'ADMIN'] },
    { label: 'Devices in Stock', value: stats.devices, icon: <Smartphone className="text-green-600" />, trend: 'Available assets', roles: ['IT', 'DIRECTOR', 'ADMIN'] },
    { label: 'Active Repairs', value: stats.repairs, icon: <Wrench className="text-amber-600" />, trend: 'Pending IT/Admin' },
    { label: 'Pending Procurements', value: stats.procurements, icon: <AlertCircle className="text-red-600" />, trend: 'Approval pipeline' },
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-6">
              ...
              {(userRole === 'IT' || userRole === 'DIRECTOR') && (
                <button onClick={() => setShowAllModal(true)} className="text-xs text-harisco-blue font-bold hover:underline">View All</button>
              )}
            </div>
          <div className="space-y-6">
            {activities.length > 0 ? activities.map((act) => (
              <div key={act.id} className="flex gap-4 group">
                <div className={`w-2 h-2 mt-1.5 rounded-full shrink-0 ${act.action === 'LOGOUT' ? 'bg-red-400' : 'bg-harisco-blue'} group-hover:scale-125 transition-transform`}></div>
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <p className="text-sm text-slate-800 leading-snug">
                      <span className="font-bold text-slate-900">{act.action}:</span> {act.details}
                    </p>
                    <span className="text-[10px] text-slate-400 font-medium ml-2 whitespace-nowrap">{formatRelativeTime(act.timestamp)}</span>
                  </div>
                  <p className="text-[10px] text-harisco-blue font-bold uppercase tracking-wider mt-1">Performed by: {act.performedBy || 'System'}</p>
                </div>
              </div>
            )) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Detailed history of all actions performed in the portal.</p>
              </div>
              <button onClick={() => setShowAllModal(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
            ...
            <div className="p-6 bg-white border-b border-slate-100 flex gap-4 items-end">
              <div className="w-64">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Time Range</label>
              ...
              <div className="w-56">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Activity Type</label>
                <select 
              ...
              <Button variant="secondary" className="px-6" onClick={() => { setFilterType('ALL'); setFilterTimeRange('ALL'); }}>
                Clear Filters
              </Button>
            </div>
                    ...
                    <tr key={act.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full ${
                          act.action.includes('REJECT') || act.action === 'LOGOUT' ? 'bg-red-100 text-red-700' : 
                          act.action.includes('CREATE') || act.action === 'LOGIN' ? 'bg-green-100 text-green-700' :
