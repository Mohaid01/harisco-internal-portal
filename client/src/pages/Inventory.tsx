    ...
    await withLoading(async () => {
      try {
        const url =
          modalMode === 'add'
            ? `${API_BASE}/inventory`
            : `${API_BASE}/inventory/${selectedDevice?.id}`
            ...
            {isLoading ? (
              ...
              filteredDevices.map((device) => (
                <tr key={device.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${getStatusStyle(device.status)}`}
                    >
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {modalMode === 'add' ? 'Register a new asset.' : 'Update device information.'}
                </p>
              </div>
              <button
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-3 bg-amber-50 text-amber-700 rounded-lg text-xs border border-amber-100 flex gap-2 italic">
                ...
                <span>
                  This action will change the device status to <b>ISSUED</b> and log the
                  transaction.
                </span>
              </div>
            </div>
