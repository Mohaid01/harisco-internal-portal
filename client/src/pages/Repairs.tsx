              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-slate-100">
                  ...
                  <div className="space-y-4 relative ml-3 before:absolute before:left-0 before:top-2 before:bottom-2 before:w-px before:bg-slate-200">
                    ...
                    <div
                      className={`relative pl-6 flex items-center justify-between ${['PENDING_IT'].includes(selectedRepair.status) ? 'text-slate-300' : ''}`}
                    >
                      <div
                        className={`absolute left-[-4px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full ${selectedRepair.status === 'PENDING_ADMIN' ? 'bg-harisco-blue animate-pulse' : ['PENDING_IT'].includes(selectedRepair.status) ? 'bg-slate-200' : 'bg-green-500'}`}
                      ></div>
                      <span className="text-xs font-semibold">Admin Approval</span>
                      <span className="text-[10px] font-bold uppercase">
                        {selectedRepair.status === 'PENDING_ADMIN'
                          ? 'Active'
                          : ['PENDING_IT'].includes(selectedRepair.status)
                            ? 'Pending'
                            : 'Completed'}
                      </span>
                    </div>
                    ...
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-100 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 leading-tight mt-0.5">
                          ...
                          {selectedRepair.status.replace('PENDING_', '')}).
                        </p>
                      </div>
                    </div>
                  )}
